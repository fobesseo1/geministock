...
export interface AlgorithmResult {
  ...
  metric_name?: string; // e.g., "PEG", "Graham Number", "200D MA", "RS Rating"
  metric_value?: number | null; // Numeric value of the metric

  // Optional: Trend status for Druckenmiller/O'Neil (추세 추종 전략용)
...
/**
 * [NEW] Strategy Group Definition
 * Defines the structure for the 3 consolidated investment styles
 */
export interface StrategyGroup {
  name: string; // e.g., "Value Hunter", "Market Timer"
  score: number; // 0-100 consolidated score
  verdict: Verdict; // Consolidated verdict
  description: string; // e.g., "Undervalued with safety margin"
}
...
export interface InvestmentSummary {
  ...
  // [NEW] 3 Investment Styles Breakdown
  strategies: {
    value: StrategyGroup; // Buffett + Graham
    growth: StrategyGroup; // Lynch + Fisher
    trend: StrategyGroup; // Druckenmiller + O'Neil
  };
}
...
export interface InvestmentAnalysisResult {
  ...
  // [NEW] Summary of all 6 opinions including strategies
  ...
  results: {
    ...
    oneil: AlgorithmResult; // [CHANGED] Replaced 'marks' with 'oneil'
  };
}
